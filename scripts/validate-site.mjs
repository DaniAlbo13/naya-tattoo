...
try{
  ...
  fail.push(`site-config.js syntax/runtime error: ${err.message}`);
}
...
for(const file of pages){
  ...
  for(const m of html.matchAll(/href="([^"]+)"/g)){
    ...
    if(href.startsWith('#')){
      ...
      if(target && !idSet.has(target)) fail.push(`${file}: broken in-page anchor -> ${href}`);
      continue;
    }
    const [pathPart,hash='']=href.split('#');
    const clean=pathPart.split('?')[0];
    if(clean && !fs.existsSync(clean)){
      fail.push(`${file}: broken local link -> ${href}`);
      continue;
    }
    if(hash && clean && /\.html$/i.test(clean)){
      const targetHtml=fs.readFileSync(clean,'utf8');
      const targetIds=new Set([...targetHtml.matchAll(/\bid="([^"]+)"/g)].map(x=>x[1]));
      if(!targetIds.has(decodeURIComponent(hash))) fail.push(`${file}: broken cross-page anchor -> ${href}`);
    }
  }
}
...
console.log(`NAYA validation passed: ${count} designs, required pages/config/assets/anchors/SEO metadata verified.`);
